//! W2 — ARC 솔버. **🧊 2026-08-15 동결(FROZEN) — 출처: `HUMAN_DERIVED`.**
//!
//! # 동결 고지 (읽고 나서 편집하려거든 멈출 것)
//!
//! 이 파일의 어휘(GridOp·객체 클래스·슬롯·휴리스틱)는 **사람이 ARC 문제를 보고
//! 발견한 규칙을 코드화한 수제 DSL**이다. MONAD가 배운 것이 아니다. 감사(시도
//! 134)에서 이 사실이 확인됐고, 운영자 지시로 다음이 **금지**된다:
//!
//! - 개별 ARC 과제를 판독해 그 규칙을 새 연산으로 작성하는 것
//! - 실패 버킷을 겨냥한 전용 GridOp/클래스/슬롯을 추가하는 것
//! - 평가셋을 분석해 설계에 반영하는 것 (**평가셋은 봉인**)
//! - 게이트 수치를 맞추려 어휘를 늘리는 것
//!
//! 이 모듈은 이제 **기질(substrate)과 baseline**으로만 존재한다: ①비교 기준선
//! ②경험 생성기(풀이 성공/실패가 [`monad_core::abstraction`]의 원료가 된다).
//!
//! # 대신 무엇을 하는가
//!
//! M2의 목적이 재정의됐다 — "코드를 한 줄도 더하지 않은 채, 경험→수면→추상화→
//! 스키마→재사용으로 MONAD가 스스로 나아지는가". 개선은 오직
//! `MONAD_DERIVED` 구조에서만 나와야 하며, KPI는 ARC 점수가 아니라 **코드 고정
//! 학습 이득·스키마 재사용률·신규 재구체화율·압축률·전이 이득·탐색 감소·
//! MONAD_DERIVED 어블레이션 낙폭**이다.
//!
//! 동결 시점 좌표(baseline): 훈련 105/400 = 26.2% · 평가 42/400 = 10.5%.
